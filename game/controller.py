...
class GameController:
    ...
    def _make_ai_move(self):
        ...
        action = raw_action if raw_action in valid_actions else random.choice(valid_actions)
        ...
        # Вызов execute_round (модель возвращает attack_val, defense_val)
        attack_val, defense_val = self.battle.execute_round(
            self.battle.team_agent, self.battle.team_player, action
        )

        self._log_round(attacker, defender, attack_val, defense_val,
                        att_str_before, def_str_before, is_ai=True)

        self._after_move()

    # ------------------------------------------------------------
    # Ход игрока (общий для PvE и PvP)
    # ------------------------------------------------------------
    def on_player_action(self, attacker_idx: int, defender_idx: int):
        if self.battle.winner:
            return

        # Определяем, какая команда атакует, а какая защищается
        if self.mode == 'pve':
        ...
        # Проверка, что выбранные бойцы живы
        if not attacker_team.fighters[attacker_idx].alive:
        ...
        self._log_round(attacker, defender, attack_val, defense_val,
                        att_str_before, def_str_before, is_ai=False)

        self._after_move()

    # ------------------------------------------------------------
    # Логирование раунда (без дублирования)
    # ------------------------------------------------------------
    def _log_round(self, attacker, defender, attack_val, defense_val,
                   att_str_before, def_str_before, is_ai: bool):
    ...
    # ------------------------------------------------------------
    # Обновление после хода (переключение игрока, проверка победителя)
    # ------------------------------------------------------------
    def _after_move(self):
        if self.battle.winner:
            self.view.update_game_screen(
                self.battle.team_agent, self.battle.team_player,
                self.current_player, self.battle.winner
            )
            return
